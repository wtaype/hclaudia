import{$ as i,N as o,j as w,z as D,d as v,a as u,A as h,B as E,i as I,x as H,b as k,c as y,l as $,t as O,y as m,q as S,e as A,h as N,v as M,M as x}from"./widev-BKqpaDEY.js";import{a as C}from"./habEstado-Djg8uXZI.js";let r=[],d=null;async function p(a){d=a;try{const t=H("habitacionesControl");if(t&&t.length>0){r=t,f();return}r=(await k(y(u,"habitaciones"))).docs.map(e=>({id:e.id,...e.data()})),$("habitacionesControl",r,300),f()}catch(t){console.error("Error carga:",t),o("Error al cargar habitaciones","error")}}function f(){const a=`
    ...
    `;i("#contenedorHabitaciones").html(a)}function z(a,t=!1){const n={libre:"#10b981",ocupado:"#ef4444",limpieza:"#06b6d4",fuera:"#f59e0b"},e=c=>{if(!c)return"-";const s=c.toDate?c.toDate():new Date(c);return s.toLocaleDateString("es-PE")+" "+s.toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"})};if(t){const c=s=>{if(!s)return"";try{return(s.toDate?s.toDate():new Date(s)).toISOString().slice(0,16)}catch{return""}};return`
    ...
    `}window.editarHabitacionControl=function(a){const t=r.find(n=>n.id===a);t&&(i(`tr[data-id="${a}"]`).replaceWith(z(t,!0)),i("#tipoHabitacion").focus(),o("Editando habitación...","info"))};window.cancelarEdicionHabitacion=function(){f(),o("Edición cancelada","info")};window.guardarCambiosHabitacion=async function(a){const t=i("#tipoHabitacion").val(),n=i("#clienteHabitacion").val().trim(),e=i("#checkInHabitacion").val(),c=i("#checkOutHabitacion").val(),s=i("#estadoHabitacion").val();if(!t)return o("Tipo obligatorio","error");try{const l={tipo:t,cliente:n||null,estado:s,checkIn:e?new Date(e):null,checkOut:c?new Date(c):null,actualizadoPor:d,fechaActualizado:w()};await D(v(u,"habitaciones",a),l),h("habitacionesControl"),await p(d),await C(),o("Habitación actualizada","success")}catch(l){console.error("Error guardar:",l),o("Error al guardar","error")}};window.eliminarHabitacionControl=async function(a){const t=r.find(n=>n.id===a);if(t&&confirm(`¿Eliminar habitación ${t.numero}?

Esta acción no se puede deshacer.`))try{await E(v(u,"habitaciones",a)),h("habitacionesControl"),await p(d),o("Habitación eliminada","success")}catch(n){console.error("Error eliminar:",n),o("Error al eliminar","error")}};window.agregarNuevaHabitacion=function(){const a=prompt("Número de habitación (ej: 101, 102, 201):");if(!a||!a.trim())return;const t=a.trim();if(r.some(e=>e.numero===t||e.id===t))return o("Habitación ya existe","error");const n=`
    ...
    `;r.length>0?i("tbody").append(n):i("tbody").html(n),i("#tipoHabitacion").focus(),o("Agregando nueva habitación...","info")};window.guardarNuevaHabitacionControl=async function(a){const t=i("#tipoHabitacion").val(),n=i("#clienteHabitacion").val().trim(),e=i("#checkInHabitacion").val(),c=i("#checkOutHabitacion").val(),s=i("#estadoHabitacion").val();if(!t)return o("Tipo obligatorio","error");try{const l={numero:a,tipo:t,cliente:n||null,estado:s||"libre",checkIn:e?new Date(e):null,checkOut:c?new Date(c):null,registradoPor:d,actualizadoPor:d,fechaActualizado:w()};await I(v(u,"habitaciones",a),l),h("habitacionesControl"),await p(d),await C(),o(`Habitación ${a} creada`,"success")}catch(l){console.error("Error crear:",l),o("Error al crear habitación","error")}};window.cancelarNuevaHabitacion=function(){r.length>0?i(".new-habitacion-row").remove():f(),o("Nueva habitación cancelada","info")};window.actualizarHabitacionesFromDB=async function(){h("habitacionesControl"),await p(d),o("Habitaciones actualizadas","success")};let b=null;O(N,async a=>{if(!a)return window.location.href="/";b=a;try{const t=H("wiSmile");if(t)return g(t),m(u,b);const e=(await k(S(y(u,"smiles"),A("usuario","==",a.displayName)))).docs[0].data();$("wiSmile",e,450),g(e),m(u,b)}catch(t){console.error(t)}});i(document).on("click",".bt_salir",async()=>{await M(N),window.location.href="/";try{localStorage.clear()}catch{Object.keys(localStorage).forEach(t=>localStorage.removeItem(t))}});function g(a){x("Bienvenido "+a.nombre+"!"),i(".app").html(`
